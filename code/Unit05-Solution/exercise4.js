...
dbRead.documents.read(uri).result(
  function(documents){
    documents.forEach(function(document){
      ...
      // We want to loop over an array of artists, so let's get all the artist elements.
      // In our example, this is really only 1 artist since we are interacting with just a single document.
      var artists = doc.getElementsByTagName("artist");

      // But we'll build a loop around it, because we may one day return multiple documents from the results of a search.
